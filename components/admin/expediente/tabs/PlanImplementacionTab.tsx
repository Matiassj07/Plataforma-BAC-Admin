...
import { crearPlan, actualizarPlan, eliminarPlan } from "@/lib/admin/plan-implementacion-actions";
import type { PlanImplementacion } from "@/lib/admin/expediente";
...
export function PlanImplementacionTab({ clienteId, planes, actividadesRat, actividadesRiesgo, documentos, focusPlanId }: Props) {
  ...
  const [expandedPlans, setExpandedPlans] = useState<Set<string>>(new Set(focusPlanId ? [focusPlanId] : []));
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-lg font-bold text-gray-900">Plan de Implementación</h2>
      ...
      <Section
        ...
        onCreate={() => setCreando("documentos")}
        expandedPlans={expandedPlans}
        onTogglePlan={togglePlan}
        clienteId={clienteId}
      />
...
function PlanCard({
  ...
  function handleEstadoChange(nuevoEstado: string) {
    startTransition(async () => {
      await actualizarPlan(plan.id, clienteId, { estado: nuevoEstado });
    });
  }
  ...
  function handleEliminar() {
    ...
    startTransition(async () => {
      await eliminarPlan(plan.id, clienteId);
    });
  }
  ...
  function handleGuardarEdicion() {
    ...
    startTransition(async () => {
      await actualizarPlan(plan.id, clienteId, {
